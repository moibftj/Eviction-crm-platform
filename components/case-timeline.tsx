...
export function CaseTimeline({ caseData }: CaseTimelineProps) {
  ...
  const timelineEvents = [
    ...
    ...caseData.tasks.map((task: any) => ({
      ...
      date: task.completed ? new Date(task.updatedAt) : new Date(task.createdAt),
      icon: task.completed ? <CheckCircle2 className="h-4 w-4" /> : <Clock className="h-4 w-4" />,
    })),
    // Notices
    ...
    // Stage changes (we don't have this data directly, but we can add it if needed)
  ]
